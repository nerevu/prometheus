...
try {
	...
	// command argument
	$source_db_name 	= $result->args['source_db_name'];
	...
	if(!$source_schema_file) {
		...
		if($connection->mdb2) {
			$arr_definition = $connection->xml_to_def_arr($source_schema_file);
	...
	if(!$errors) {
		if($arr_definition) {
			if($output_schema_file) {
				...
				// strip path from sqlite database name
				if(($source_db_type == 'sqlite') && (($dest_db_type != 'sqlite') || isset($output_schema_file))) {
					$dest_db_name = $general->get_base($dest_db_name);
				} } //<-- end if -->
								
				$xml_file = $schema_path.$dest_db_name.'.xml';
				$check = $general->write2file($xml_data, $xml_file);
	...
	//
	// mysql -> schema file		| sqlite2pgsql -fvs mysql -p stockman04 test			| go
	// postgres -> schema file	| sqlite2pgsql -fvs pgsql -p stockman04 test			| go
	// sqlite -> schema file	| sqlite2pgsql -fv /Users/reubano/Databases/test.sqlite
