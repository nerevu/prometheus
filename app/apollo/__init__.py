...
{<Timestamp: 2013-01-01 00:00:00>: 1}}
	...
	def join_shares3(self, other, common=['date', 'commodity_id']):
		"""Examples
		--------
		>>> from datetime import datetime as dt
		...
		>>> mp.join_shares3(df).to_records()[0]
		(datetime.datetime(1970, 1, 16, 48, 0), 1, 34.0)
		"""
		return other

	def join_shares2(self, other, common=['date', 'commodity_id']):
		"""Examples
		--------
		>>> from datetime import datetime as dt
		>>> mp = Portfolio(DataObject())
		...
		>>> data = [(dt(2013, 1, 1), 1, 34.)]
		>>> df = DataObject(data, keys=keys)
		>>> mp.join_shares2(df).to_records()[0]
		(0, datetime.datetime(1970, 1, 16, 48, 0), 1, 34.0, 1, 1, \
datetime.datetime(1970, 1, 16, 48, 0), 0)
		"""
		return other.merge_frame(self.shares, 'commodity_id')

	def join_shares1(self, other, common=['date', 'commodity_id']):
		"""Examples
		--------
		>>> from datetime import datetime as dt
		>>> mp = Portfolio(DataObject())
		...
		>>> data = [(dt(2013, 1, 1), 1, 34.)]
		>>> df = DataObject(data, keys=keys)
		>>> mp.join_shares1(df).to_records()[0]
		((1, 1, 1, <Timestamp: 2013-01-01 00:00:00>), 34.0, 0)
		"""
		...
		return merged.join_merged(index, x, y)
