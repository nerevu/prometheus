...
newevent = {'symbol': 'ISIS', 'type': {'name': 'Dividend', 'unit': 'USD'},
	'value': 100}
r = requests.post('%s/event' % site, data=json.dumps(newevent),
	headers=headers)
r.status_code, r.headers['content-type'], r.json

newevent = {'symbol': 'ISIS', 'date': '1/22/12', 'type': {'id': 2},
	'value': 100}
r = requests.post('%s/event' % site, data=json.dumps(newevent),
	headers=headers)
r.status_code, r.headers['content-type'], r.json
...
r = requests.post('%s/type' % site, data=json.dumps(neweventtype),
	headers=headers)
r.status_code, r.headers['content-type'], r.json
